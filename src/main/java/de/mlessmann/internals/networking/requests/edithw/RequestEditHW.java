package de.mlessmann.internals.networking.requests.edithw;
 ...
 */
public class RequestEditHW implements IRequest, IMessageListener, IHWFutureProvider<Boolean> {
    ...
    public RequestEditHW(LMgr logger) {
        lMgr = logger;
        genID();
    ...
    public boolean onMessage(JSONObject msg) {
        ...
        if (msg.optString("payload_type", "null").equals("error")) {
            ...
            else if (err.equals(Errors.EditHWError)) {
                if (status == 400)
